...
#include "utils.h"
__global__ void fftshift_2d_power2_kernel(
    const cuComplex* __restrict__ input,
    cuComplex* __restrict__ output, int width, int height) {
...
void fftshift(XMux<ComplexMatrix>& A) {
  ...
  fftshift_2d_power2_kernel<<<GRID_SIZE,BLOCK_SIZE>>>((const cuComplex*)A.device_data(),(cuComplex*)d_tmp, A.getSize1(), A.getSize2());
  CUDA_CHECK(cudaMemcpy(A.device_data(), d_tmp,sizeof(ComplexMatrix::dtype)*A.getSize(), cudaMemcpyDeviceToDevice));
...
void ifftshift(XMux<ComplexMatrix>& A) {
  ...
  void* d_tmp;
  CUDA_CHECK(cudaMalloc(&d_tmp,sizeof(ComplexMatrix::dtype)*A.getSize()));
  ifftshift_2d_pow2_kernel<<<GRID_SIZE,BLOCK_SIZE>>>((const cuComplex*)A.device_data(),(cuComplex*)d_tmp, A.getSize1(), A.getSize2());
  CUDA_CHECK(cudaMemcpy(A.device_data(), d_tmp,sizeof(ComplexMatrix::dtype)*A.getSize(), cudaMemcpyDeviceToDevice));
  CUDA_CHECK(cudaFree(d_tmp));
}